...
ask_project() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -*)
        ...
        ;;
      *)
        if [[ ${#projects[@]} -eq 0 ]] && resolve_project_dir "$workspace" "$1" >/dev/null 2>&1; then
          projects+=("$1")
        else
          question="$*"
          break
  ...
  for project in "${projects[@]}"; do
    local project_dir
    if project_dir=$(resolve_project_dir "$workspace" "$project"); then
      claude_args+=(--add-dir "$project_dir")
    else
      log_error "skipping unsafe project: $project" "ask"
      return 1
    fi
  done
  ...
  for project in "${projects[@]}"; do
    local project_dir
    project_dir=$(resolve_project_dir "$workspace" "$project") || continue
