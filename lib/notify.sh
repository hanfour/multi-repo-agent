...
_send_slack() {
  ...
  # TM-008: refuse to POST anywhere policy rejects.
  if ! check_safe_url "$url"; then
    log_warn "slack notification skipped (policy rejected URL)" "notify"
    return 0
  fi
  local color
  case "$level" in
  ...
  curl -sf "${curl_safe[@]}" -X POST -H "Content-Type: application/json" -d "$payload" "$url" &>/dev/null && \
...
_send_discord() {
  local url="$1" event="$2" message="$3" level="$4"
  if ! check_safe_url "$url"; then
    log_warn "discord notification skipped (policy rejected URL)" "notify"
    return 0
  fi
  local color
  case "$level" in
...
_send_generic() {
  local url="$1" event="$2" message="$3" level="$4"
  if ! check_safe_url "$url"; then
    log_warn "webhook notification skipped (policy rejected URL)" "notify"
    return 0
  fi
