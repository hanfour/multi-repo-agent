...
GITIGNORE
  ...
  # TM-004: refuse to auto-trigger dump downloads and SQL imports in
  # non-interactive contexts (CI, MCP, scripts). An attacker-controlled
  # db.json that lands in the workspace before `mra init` runs would
  # otherwise pull arbitrary URLs and execute SQL inside a privileged
  # container without operator consent. Set MRA_INIT_AUTO_DB=1 to opt
  # back into the old behaviour.
  if db_json_exists "$workspace"; then
    if [[ -t 0 || "${MRA_INIT_AUTO_DB:-}" == "1" ]]; then
      log_success "db.json found, setting up databases" "init"
      setup_all_databases "$workspace"
    else
      log_warn "db.json found, but stdin is not a terminal; skipping DB setup (set MRA_INIT_AUTO_DB=1 to override)" "init"
    fi
  else
    if command -v docker &>/dev/null; then
      if [[ -t 0 ]]; then
        interactive_db_setup "$workspace"
        if db_json_exists "$workspace" && [[ -t 0 || "${MRA_INIT_AUTO_DB:-}" == "1" ]]; then
          setup_all_databases "$workspace"
        fi
      else
        log_warn "stdin is not a terminal; skipping interactive DB setup" "init"
      fi
    else
