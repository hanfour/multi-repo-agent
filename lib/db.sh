...
_resolve_source() {
  ...
  if [[ "$source" == http://* || "$source" == https://* ]]; then
    # TM-004 / TM-008: refuse loopback / private IP / non-HTTPS URLs.
    # Operators who legitimately host dumps internally can override
    # with MRA_ALLOW_LOCAL_ENDPOINTS=1 / MRA_ALLOW_HTTP=1.
    if ! check_safe_url "$source"; then
      return 1
    fi
    # DB dumps can be large; bump the default 5MB cap. Operators who
    # want a smaller hard ceiling can set MRA_DB_DUMP_MAX_BYTES.
    local max_bytes="${MRA_DB_DUMP_MAX_BYTES:-2147483648}"  # 2 GB default
    local filename
    filename=$(basename "$source")
    # Sanitize the filename so an attacker-controlled URL cannot
    # produce a path with shell metacharacters or traversal segments
    # when joined under /tmp.
    filename="${filename//[^A-Za-z0-9._-]/_}"
    local tmp="/tmp/mra_download_${filename}"
    log_progress "downloading dump from $source" "db"
    if ! curl -fsSL --max-time 300 --max-filesize "$max_bytes" "$source" -o "$tmp"; then
      log_error "dump download failed (size/time cap, network error, or HTTP error): $source" "db"
      rm -f "$tmp"
      return 1
    fi
    echo "$tmp"
  elif [[ "$source" == *"://"* ]]; then
    # Any other URL scheme (file://, ftp://, ...) is rejected outright
    # so a hostile db.json cannot trick _resolve_source into reading
    # arbitrary files.
    log_error "unsupported dump source scheme: $source" "db"
    return 1
  elif [[ "$source" != /* ]]; then
