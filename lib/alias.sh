...
handle_alias() {
  ...
  # Timestamped backup before any modification (TM-010). sed -i.bak only
  # keeps a transient backup that we delete immediately; keep a persistent
  # one so the operator can recover.
  if [[ -f "$shell_rc" ]]; then
    local backup_ts backup_file
  ...
  # Quote workspace and mra_dir before embedding (TM-010). A naive
  # `MRA_WORKSPACE="$workspace"` heredoc would expand `$` characters in the
  # path at source time and could break out of the quoted string if the
  # path contains `"`. printf '%q' produces a shell-safe token.
  local workspace_q mra_dir_q
...
$name() {
  MRA_WORKSPACE=${workspace_q} ${mra_dir_q}/bin/mra.sh "\$@"
}
# mra-alias:$name end
SHELL
