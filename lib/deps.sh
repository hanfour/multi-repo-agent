...
get_project_consumers() {
  local project="$1" graph_file="$2"
  # TM-003: pass project via --arg so a hostile name cannot inject jq
  # syntax. The earlier interpolation form would break the filter (and
  # in unrelated callers could lead to argument confusion).
...
display_deps() {
  ...
  # TM-003: --arg instead of string interpolation.
  deps_infra=$(jq -r --arg p "$project" '.projects[$p].deps.infra // [] | join(", ")' "$graph_file")
  deps_api=$(jq -r --arg p "$project" '.projects[$p].deps.api // [] | join(", ")' "$graph_file")
