...
subscribe_contract() {
  ...
  # TM-008: validate URL before persisting it so a tampered
  # subscriptions.json cannot be planted on disk via an unsafe URL.
  # Local files (no scheme) are still accepted.
  if [[ "$source_url" == http://* || "$source_url" == https://* ]]; then
    if ! check_safe_url "$source_url"; then
      return 1
    fi
  fi
  local contracts_dir; contracts_dir=$(get_contracts_dir "$workspace")
...
fetch_subscription() {
  ...
  if [[ "$source_url" == http://* || "$source_url" == https://* ]]; then
    # TM-008: enforce HTTPS / private-IP / size+time limits.
    if ! check_safe_url "$source_url"; then
      return 1
    fi
    local curl_safe
    mapfile -t curl_safe < <(safe_curl_args)
    contract_content=$(curl -sf "${curl_safe[@]}" "$source_url" 2>/dev/null) || {
      log_error "failed to fetch: $source_url" "federation"
      return 1
    }
  elif [[ -f "$source_url" ]]; then
    # Local file paths are intentionally separate from the URL policy.
    # Operators reading a checked-in contract.json on disk are not at
    # SSRF risk; mra federation subscribe http(s)://... is where the
    # policy applies.
