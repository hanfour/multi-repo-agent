...
| `lint-profile.json` | lint 규칙 세트 선택 (`{"profile":"legacy"}` 또는 인라인 `rules`) | 예 |
...
> **⚠ 마이그레이션 안내 (lint 기본값 변경)**: 이전 버전은 `lib/lint.sh`에 Acme BLOCKER 규칙이 하드코딩되어 있었습니다. 이제 lint는 profile 기반이며 기본 profile은 비어 있습니다. 이전 동작을 유지하려면 워크스페이스에 한 줄 파일을 추가하세요:
> ```bash
...
| `legacy` | Acme 프론트엔드 BLOCKER 규칙 (no-interface / no-enum / no-any / no-non-null / no-var) |
