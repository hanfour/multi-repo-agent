...
| `lib/prd.sh` | **create** | `_prd_alloc_req_id` (atomic REQ-YYYY-NNNN via `mkdir`-lock in `.collab/requirements/.locks/`, max+1 scan, retry on EEXIST, per-year reset, on-exit cleanup of an empty reservation); pure `_prd_build_launch_argv`; `prd_launch` (export `MRA_PRD_REQ_ID` + `MRA_PRD_PROJECTS` (resolved scope) for the in-session agent, AND **persist the resolved scope to a `<REQ>-scope` sidecar** under `.collab/requirements/` so the separate apply shell can read the true launch scope; build abs-workspace + abs-`mra`-path + "present-and-stop" narration fragments; `( cd "$workspace" && _launch_interactive agents/prd-agent.md … )` interactive). |
...
1. **Validate** the Task-Plan JSON before any `gh` call (D14): each task has `id/project/tier/dependencies/acceptance_criteria`; every `task.project` is a member of `MRA_PRD_PROJECTS` — which the apply dispatch reads from the **`<REQ>-scope` sidecar written by `prd_launch` at plan time** (the resolved launch scope), NOT defaulted to all workspace repos and NOT trusted from an inherited env or the agent. Missing sidecar ⇒ abort ("was it created by `mra prd`?"). So blast radius cannot widen at apply time. Every `dependencies` id exists in the plan; `requirement_id == --req`. Abort cleanly naming the bad field; no self-heal.
...
- **Scope sidecar** — `<workspace>/.collab/requirements/REQ-YYYY-NNNN-scope` (written by `prd_launch` at plan time; the resolved launch scope the apply step trusts for the §7.1 bound).
...
3. **(MED)** `task.project` scope guard degraded to existence-only. **Closed** by persisting the resolved launch scope to a `<REQ>-scope` sidecar at plan time and having the apply path read it (never defaulting to all workspace repos), then asserting membership (§7.1). *(Final whole-branch review caught that an env-only scope silently widened to all workspace repos in the operator's separate apply shell; the sidecar restores the bound.)*
