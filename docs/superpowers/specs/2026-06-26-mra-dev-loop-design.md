...
- **D7 — fresh-diff re-review.** ~~Export `MRA_REVIEW_PR_CONTEXT=0` on loop-internal re-reviews so the bot's own prior comments aren't fetched-and-suppressed into a cross-round false green.~~ **Superseded (2026-08-11).** The hazard was the *framing*, not the fetch: the generic block is headed "do NOT re-report issues already raised here", and the loop's own round-1 findings arrived through it, so round 2 stayed silent and the loop read silence as a fix. Our own prior output is now presented separately as ours — judged against the current diff, re-reported if it still holds, and adjudicated when somebody replied (`lib/review-pr-threads.sh`). With the false green structurally gone, the loop reads PR context like any other `--pr` review, which is what lets a reviewer's rebuttal reach it at all. `tests/test_dev_pr_context.sh` asserts both halves: the loop no longer blinds itself, AND its own finding never lands under do-not-re-report.
...
False-green firewall (verdict only from validated result-file status; missing/empty/invalid ⇒ `REVIEW_INCOMPLETE`, never `APPROVED`) · force `MRA_REVIEW_VERIFY_APPROVE=1` · GitHub APPROVE opt-in only · our own prior findings kept out of the do-not-re-report block (D7, superseded) · empty-diff guard before every review · unconditional top-of-loop push · scoped write grant (no `acceptEdits`/`--dangerously-skip`) · no `git add -A` · fork from `origin/<default>` · cwd+`GH_TOKEN` pinned · all logs on stderr · `|| true` guards under `set -e`.
