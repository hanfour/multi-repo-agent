...
兩組數字，同一組 38 個 PR、53 條 expected finding、同一個 `candidates_sha`
（`18dd5485f5b50463`）、零排除。
...
**團隊今天的 code review，在 38 個真實 PR 上抓到 53 條已知缺陷中的 6 到 10 條。**
...
基準集的 53 條裡只有 5 條屬於測試品質，其餘偏向功能缺陷。所以 C 的產出與基準集
...
另外，53 條 finding 只對應約 33 個相異缺陷。「草稿沒依 lineItem.id 分」一個缺陷跨
