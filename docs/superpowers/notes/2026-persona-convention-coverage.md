...
第一，**目前是單邊判斷**。codex 側的交叉驗證還在跑（它逐條查 git 歷史，
比 Claude 側的平行 agent 慢得多）。先前幾次交叉驗證的一致率是 99%（是不是
同一件事）、80%（符不符合 Rule 4／5）、100%（B 類 5 條的有效性），這次的
任務形態最接近最後一種，但沒有數字之前不該當定論。
...
一、等 codex 側查證完成，比對一致率。一致率高就把清理結果定案；分歧多
就逐條裁決分歧的部分。

二、定案之後，`candidates.json` 要實際落地清理（標記或移除那 34 條），
