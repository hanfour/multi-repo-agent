...
**file_missed 的浮動是 3 條**（23 對 26）。這正是先前一直缺的那個數字，
而四輪（24、25、26、23）之間的全部差距也是 3 條。
...
- 〈驗證：personas 路徑 synthesize 專屬 turn 上限〉說 file_missed 23
  ...
  量出來了：浮動 3 條，23 到 26 全部在裡面，那句話沒有意義。
...
一、**停止用 file_miss_rate 當這條線的主要指標。** 它的浮動（3 條）
跟四輪之間的全部差異（3 條）一樣大。要繼續用它，得先讓單輪的樣本大到
