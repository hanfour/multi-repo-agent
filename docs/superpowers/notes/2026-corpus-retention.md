...
`nest-app-2`（194）、`nest-app-3`（60），這些 repo 的 review 是團隊自己的判準，
本來就是嚴重度標準的來源。外部語料在這一層只提供有限的方法參考。

依 spec 「外面的人教方法、自己的團隊定標準」的分工，第三個做法與設計意圖最一致，但它意味著
NestJS 層的「方法」那一半會比其他層薄。

**已決定採用第三個做法**（2026-08-15）。NestJS 層的規則以自家語料為主，外部語料只補方法層面的
參考。spec 已加上這一層的例外條款與三個具體影響，見設計文件的「NestJS 層的例外」。
