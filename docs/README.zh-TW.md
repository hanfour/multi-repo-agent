...
| `lint-profile.json` | 選擇 lint 規則集（`{"profile":"legacy"}` 或內嵌 `rules`） | 是 |
...
> **⚠ 遷移提示（lint 預設已變更）**：早期版本在 `lib/lint.sh` 寫死 Acme BLOCKER 規則。lint 現在改為 profile 驅動，預設 profile 為空。要恢復原行為，在工作區建立一行檔案：
> ```bash
...
| `legacy` | Acme 前端 BLOCKER 規則（no-interface / no-enum / no-any / no-non-null / no-var） |
