...
//
// `PROJECT_NAME_PATTERN` mirrors the allowlist in `lib/project-path.sh`
// (`validate_project_name`). Keeping the regex in sync is critical:
// it is the MCP-side line of defense against `../`, absolute paths,
// and other path-confusion inputs. Any change here MUST be applied to
// `lib/project-path.sh` as well, or the two layers will disagree.
...
export const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        workspace: {
          type: "string",
          description: "Workspace root path (e.g., /Users/you/Acme)",
        },
      },
      required: ["workspace"],
    },
