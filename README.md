...
**v3.4.1** | 42 CLI commands | 6 AI agents | 9 MCP tools | 111 test suites | 10 TM-tracked security controls
...
| `MRA_REVIEW_SYNTH_MAX_TURNS` | `8` | Turn budget for the **synthesize** step that merges findings into the final review. Shared by the debate path (2 agent outputs) and the personas path (all persona outputs concatenated). |
| `MRA_REVIEW_PERSONA_SYNTH_MAX_TURNS` | `MRA_REVIEW_SYNTH_MAX_TURNS` | Synthesize turn budget override for the **personas** path only. It merges more (and, with the coverage checklist, longer) input than the debate path's 2 agents; leave unset to keep it on the shared budget. |
| `MRA_REVIEW_PERSONA_DUMP_DIR` | unset | Directory to keep each persona's raw stdout (`<persona>.txt`) and stderr (`<persona>.err`). The final review JSON cannot distinguish "this persona never mentioned the file" from "it did, and synthesize dropped it"; the raw output can. Unset writes nothing. |
