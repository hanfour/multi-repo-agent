...
| `lint-profile.json` | Selects a lint rule set (`{"profile":"legacy"}` or inline `rules`) | Yes |
...
> Acme BLOCKER rules in `lib/lint.sh`. Lint is now profile-driven and the
...
| `legacy` | Acme frontend BLOCKER rules (no-interface / no-enum / no-any / no-non-null / no-var) |
