...
for project_dir in "$WORKSPACE"/*/; do
  ...
  if [[ -f "$gemfile" ]]; then
    while IFS= read -r line; do
      ...
      # Look for git/github-sourced (private/internal) gems, then map to known workspace projects
      if [[ "$line" =~ gem[[:space:]]+[\"\']([a-zA-Z0-9_-]+)[\"\'].*git ]]; then
  ...
  # Node: scan package.json for scoped (@scope/*) packages or workspace project names
  package_json="$project_dir/package.json"
  if [[ -f "$package_json" ]]; then
    # Extract dependency names matching @scope/* or workspace project names
    while IFS= read -r dep_name; do
      [[ -z "$dep_name" ]] && continue

      # scoped (@scope/*) packages
      if [[ "$dep_name" =~ ^@[^/]+/(.+)$ ]]; then
