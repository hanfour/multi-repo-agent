...
if [[ ! -d "$TEST_DIR/workspace/.collab" ]]; then
  echo "FAIL: .collab dir not created"; errors=$((errors+1))
fi

# Check dep-graph.json created
if [[ ! -f "$TEST_DIR/workspace/.collab/dep-graph.json" ]]; then
  echo "FAIL: dep-graph.json not created"; errors=$((errors+1))
fi
...
if [[ "$project_count" != "2" ]]; then
  echo "FAIL: should have 2 projects, got $project_count"; errors=$((errors+1))
fi
...
if [[ "$alias_workspace" != "$TEST_DIR/workspace" ]]; then
  echo "FAIL: alias not created, got $alias_workspace"; errors=$((errors+1))
fi
