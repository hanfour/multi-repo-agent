...
if [[ "$result" != "rails-api" ]]; then echo "FAIL: rails should be detected, got $result"; errors=$((errors+1)); fi
...
if [[ "$result" != "node-frontend" ]]; then echo "FAIL: node-frontend should be detected, got $result"; errors=$((errors+1)); fi
mkdir -p "$TEST_DIR/mystery"
result=$(detect_project_type "$TEST_DIR/mystery")
if [[ "$result" != "unknown" ]]; then echo "FAIL: unknown should be detected, got $result"; errors=$((errors+1)); fi
rm -rf "$TEST_DIR"
