...
if [[ "$output" != *"api"* ]]; then echo "FAIL: terminal should show api"; errors=$((errors+1)); fi
if [[ "$output" != *"frontend"* ]]; then echo "FAIL: terminal should show frontend"; errors=$((errors+1)); fi
# Test mermaid
output=$(_graph_mermaid "$TEST_DIR/.collab/dep-graph.json" "$TEST_DIR" 2>&1)
if [[ "$output" != *"graph LR"* ]]; then echo "FAIL: mermaid should start with graph LR"; errors=$((errors+1)); fi
if [[ ! -f "$TEST_DIR/.collab/dep-graph.mmd" ]]; then echo "FAIL: mermaid file not created"; errors=$((errors+1)); fi
# Test dot
output=$(_graph_dot "$TEST_DIR/.collab/dep-graph.json" "$TEST_DIR" 2>&1)
if [[ "$output" != *"digraph"* ]]; then echo "FAIL: dot should contain digraph"; errors=$((errors+1)); fi
if [[ ! -f "$TEST_DIR/.collab/dep-graph.dot" ]]; then echo "FAIL: dot file not created"; errors=$((errors+1)); fi
rm -rf "$TEST_DIR"
