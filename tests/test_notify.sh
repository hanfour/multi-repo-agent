...
# Test _level_passes
if ! _level_passes "error" "warn"; then echo "FAIL: error should pass warn filter"; errors=$((errors+1)); fi
if ! _level_passes "warn" "warn"; then echo "FAIL: warn should pass warn filter"; errors=$((errors+1)); fi
if _level_passes "info" "warn"; then echo "FAIL: info should NOT pass warn filter"; errors=$((errors+1)); fi
if ! _level_passes "critical" "info"; then echo "FAIL: critical should pass info filter"; errors=$((errors+1)); fi
...
if [[ ! -f "$notify_config" ]]; then echo "FAIL: notify.json not created"; errors=$((errors+1)); fi

# Test template is valid JSON with webhooks array
webhook_count=$(jq '.webhooks | length' "$notify_config" 2>/dev/null)
if [[ "$webhook_count" != "2" ]]; then echo "FAIL: should have 2 webhooks, got $webhook_count"; errors=$((errors+1)); fi
...
if ! type notify_test_failed &>/dev/null; then echo "FAIL: notify_test_failed not defined"; errors=$((errors+1)); fi
