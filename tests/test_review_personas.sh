...
if build_persona_prompt "bogus-persona-xyz" "d" "c" 2>/dev/null; then
  echo "FAIL: build_persona_prompt should return non-zero on unknown persona"; ((errors++))
fi
...
if [[ "$prompt_pkb" != *"SENTINEL_PKB_LINE"* ]]; then
  echo "FAIL: prompt missing PKB context"; ((errors++))
fi
...
if [[ "$prompt_cons" != *"Consumer projects: consumer-a consumer-b"* ]]; then
  echo "FAIL: prompt missing consumer section"; ((errors++))
fi
