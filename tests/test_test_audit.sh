...
if [[ -n "$output" ]]; then echo "FAIL: expected empty on missing dir"; ((errors++)); fi
...
if [[ "$output" != *"foo.test.js"* ]]; then echo "FAIL: missing foo.test.js"; ((errors++)); fi
if [[ "$output" != *"bar_test.go"* ]]; then echo "FAIL: missing bar_test.go"; ((errors++)); fi
if [[ "$output" == *"nothing.js"* ]]; then echo "FAIL: should not include nothing.js"; ((errors++)); fi
rm -rf "$tmpdir"
...
if [[ "$prompt" != *"11 PRINCIPLES"* ]]; then
  echo "FAIL: prompt missing 11 PRINCIPLES"; ((errors++))
fi
if [[ "$prompt" != *"tests/foo.test.js"* ]]; then
  echo "FAIL: prompt missing file path"; ((errors++))
fi
if [[ "$prompt" != *"describe('x')"* ]]; then
  echo "FAIL: prompt missing file content"; ((errors++))
fi
...
if [[ "$prompt_lang" != *"Use 繁體中文 for output."* ]]; then
  echo "FAIL: prompt missing language directive"; ((errors++))
fi
...
if [[ "$prompt_inj" != *'$(whoami)'* ]]; then
  echo "FAIL: file contents should be kept literal (no command substitution)"; ((errors++))
fi
