...
if ! check_tool "git" "version control"; then
  echo "FAIL: git should be found"; errors=$((errors+1))
fi
if check_tool "nonexistent_tool_xyz" "fake tool" 2>/dev/null; then
  echo "FAIL: nonexistent tool should not be found"; errors=$((errors+1))
fi
if type run_preflight &>/dev/null; then
  echo "PASS: run_preflight function exists"
else
  echo "FAIL: run_preflight not defined"; errors=$((errors+1))
fi
if [[ $errors -eq 0 ]]; then
