...
if [[ "$output" != *"PROJECTS"* ]]; then echo "FAIL: should show PROJECTS header"; errors=$((errors+1)); fi
if [[ "$output" != *"DEPENDENCY GRAPH"* ]]; then echo "FAIL: should show DEPENDENCY GRAPH header"; errors=$((errors+1)); fi
if [[ "$output" != *"api"* ]]; then echo "FAIL: should show api project"; errors=$((errors+1)); fi
if [[ "$output" != *"frontend"* ]]; then echo "FAIL: should show frontend project"; errors=$((errors+1)); fi
if [[ "$output" != *"DATABASES"* ]]; then echo "FAIL: should show DATABASES section"; errors=$((errors+1)); fi
if [[ "$output" != *"RECENT CHANGES"* ]]; then echo "FAIL: should show RECENT CHANGES"; errors=$((errors+1)); fi
