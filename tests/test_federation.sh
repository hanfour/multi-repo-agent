...
if [[ ! -f "$contract_file" ]]; then
  echo "FAIL: contract file not created"; errors=$((errors+1))
else
  ep_count=$(jq '.endpointCount' "$contract_file")
  if [[ "$ep_count" -lt 2 ]]; then
    echo "FAIL: should have at least 2 endpoints, got $ep_count"; errors=$((errors+1))
  fi
  schema=$(jq -r '.schema' "$contract_file")
  if [[ "$schema" != "mra-contract-v1" ]]; then
    echo "FAIL: schema should be mra-contract-v1, got $schema"; errors=$((errors+1))
  fi
fi
...
if [[ ! -f "$subs_file" ]]; then
  echo "FAIL: subscriptions file not created"; errors=$((errors+1))
fi
sub_count=$(jq 'length' "$subs_file")
if [[ "$sub_count" != "1" ]]; then
  echo "FAIL: should have 1 subscription, got $sub_count"; errors=$((errors+1))
fi

# Test verify
output=$(verify_contracts "$TEST_DIR" 2>&1)
if [[ "$output" != *"passed"* ]]; then
  echo "FAIL: verify should show passed"; errors=$((errors+1))
fi

# Test list
output=$(list_contracts "$TEST_DIR" 2>&1)
if [[ "$output" != *"myapi"* ]]; then
  echo "FAIL: list should show myapi"; errors=$((errors+1))
fi
