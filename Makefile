...
help:
	...
	@echo "make lint         Run shellcheck at the CI blocking severity (-S warning)"
	@echo "make lint-all     Run shellcheck at -S style (informational, not a gate)"
...
# Gates on -S warning, the same severity CI blocks on (.github/workflows/
# repo-tests.yml). These two drifted once — lint ran at -S warning and was
# permanently red on a 50-finding backlog while CI only blocked on -S error —
# and a developer command that always fails is a developer command nobody
# reads. The backlog is now clear (#50), so the gate sits at warning to stop it
...
# One level below the gate. Not clean and not expected to be — this is for
# looking, not for gating.
lint-all:
	@if command -v shellcheck >/dev/null 2>&1; then \
		shellcheck -S style lib/*.sh bin/*.sh tests/*.sh test.sh; \
	else \
		echo "shellcheck not installed (brew install shellcheck) — skipping"; \
	fi
